...
_SQRT2 = math.sqrt(2)
_SCALE_SQRT2 = np.array([_SQRT2, _SQRT2, _SQRT2],
                        dtype=np.float64)
...
def _resampling_mem_needed_mb(out_shape, scale_factor):
    ...
    mem_needed = 2 * (n_in.prod() + n_out.prod()) +\
        4 * n_out[0] * n_out[1] * n_in[2]
    return mem_needed // 2**20


def _blockwise_resample(in_im_name, out_im_name, scale_factor,
                        is_mask=False, mem_limit_mb=-1, thread_num=-1):
    """
    ...
    out_im_shape_array_xyz = np.round(
        in_im.shape_array_xyz / scale_factor).astype(np.uint32)
    ...
    out_im = image3d.create_image(out_im_name, out_im_shape,
                                  out_im_offset, out_im_spacing,
                                  in_im.transform_matrix, mode='w')
    out_im.data.flush()
    ...
    if mem_needed_mb < mem_limit_mb:
        ...
        batch_size_z = 1
        while _resampling_mem_needed_mb(
                (batch_size_z, out_im.shape[1], out_im.shape[2]),
                scale_factor) < mem_limit_mb:
            batch_size_z += 1
        batch_size_z -= 1
        if batch_size_z == 0:
        ...
        for z_pos in range(0, out_im.shape[0], batch_size_z):
            ...
            in_end = min(int(np.ceil((z_end+1)*scale_factor[2] - 1)),
                         in_im.shape[0])
            ...
            cy_ip.downsample_with_offset(in_block, out_block,
                                         tmp_array[0:in_end - in_start, :, :],
                                         scale_factor, in_offset,
                                         thread_num, is_mask)
...
def _create_image_pyramid(base_img_name, highest_resolution, is_sqrt2_pyramid,
                          nr_of_levels, is_mask, mem_limit_mb, thread_num):
    """
        Args:
    ...
    if np.allclose(base_img.spacing, highest_resolution):
        ...
        _blockwise_resample(base_img_name, out_img_name, scale_factor,
                            is_mask, mem_limit_mb, thread_num)
    ...
    for i in range(1, nr_of_levels + 1):
        ...
        logging.info(os.path.basename(out_img_name)+' :')
        _blockwise_resample(in_img_name, out_img_name, scale_factor,
                            is_mask, mem_limit_mb, thread_num)
        file_name_list.append(out_img_name)
...
def create_pyramid(reg_config):
    ...
    for n in ['fixed_grey_base', 'moving_grey_base',
              'fixed_mask_base', 'moving_mask_base']:
    ...
    base_spacing *= \
        reg_config.getfloat('multiresolution', 'pyramid_min_scale_factor')
    logging.debug('base_spacing: {} mm'.format(base_spacing))
    pyramid_sqrt2_scaling = reg_config.getboolean('multiresolution',
    ...
    for n in ['fixed_grey_base', 'moving_grey_base',
        ...
        if reg_config.has_option('pyramid', n):
            ...
            current_level_nr = int(math.log(nr_vox_at_base_level /
                                            min_vox_nr) /
                                   math.log(level_scale_factor) / 3.0 + 0.5)
            # need at least 7 as mininum nr of voxels in each direction
            dim_level_nr = int(math.log(min(img.shape)/7) /
    ...
    for n in ['fixed_grey_base', 'moving_grey_base',
        ...
        if reg_config.has_option('pyramid', n):
            ...
            file_name_list = \
                _create_image_pyramid(im_name, base_spacing,
                                      pyramid_sqrt2_scaling,
                                      level_nr, 'mask' in n,
