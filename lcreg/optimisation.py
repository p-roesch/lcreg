...
def inertia_tensor(im):
    ...
    for z in range(1, im.shape[0]-1):
        cy_ip.add_inertia_tensor_slice(im.data[z, :, :],
                                       z, im.voxel_to_world_matrix,
                                       N, centre, TI)
...
def get_initial_transform_from_moments(reg_config):
    ...
    reg_config['optimisation']['initial_transform'] = \
        repr(m_opt).replace('array', 'np.array').replace('float64',
                                                         'np.float64')
    reg_config['optimisation']['rotation_centre'] = \
        repr(c_fixed).replace('array', 'np.array').replace('float64',
                                                           'np.float64')
    reg_config['optimisation']['rotation_axes'] = \
        repr(evect_fixed).replace('array', 'np.array').\
        replace('float64', 'np.float64')
    logging.debug('initial_transform from moments: \n' + str(m_opt))
    logging.debug('initial euler angles (xyz, deg):\n' +
...
def simple_gradient(fixed_im, moving_im, p_initial, c_rot=None, v_rot=None,
                    d_v_initial=2, d_v_min=1, mem_limit_mb=-1, num_threads=-1):
    """
    ...
    # initial lcc value
    N, lcc_min = similarity.lcc(fixed_im, moving_im, p_initial,
                                False, mem_limit_mb, num_threads)
    ...
    while delta_t >= delta_t_min:
        ...
        for k in range(12):
            ...
            T_l = np.dot(T_initial, np.dot(T_c_inv,
                                           np.dot(np.linalg.inv(d_T), T_c)))
            p_l = p_from_t_matrix_world(T_l)
            N, lcc = similarity.lcc(fixed_im, moving_im, p_l, False,
                                    mem_limit_mb, num_threads)
        ...
        if lcc_min_step < lcc_min:
            ...
            logging.debug('lcc_min: {:f}, d_p_opt: {:s}'.
                          format(lcc_min,
                                 str(list(p_from_t_matrix_world(d_T_opt)))))
        else:
            delta_t = delta_t / 2.0
            delta_r = delta_r / 2.0
        n_steps += 1
    ...
    T_opt = np.dot(T_initial, np.dot(T_c_inv,
                                     np.dot(np.linalg.inv(d_T_opt), T_c)))
...
def levenberg_marquardt(fixed_im, moving_im, p_initial,
                        l_initial=1.0, l_downscale_factor=1./10.,
                        l_upscale_factor=10, max_step_num=50,
                        min_vox_delta=0.1, max_nr_of_upscale_steps=4,
                        mem_limit_mb=-1, num_threads=-1):
    """
    ...
    # initialise values
    lcc_val_opt, grad_opt, H_opt = \
        similarity.lcc(fixed_im, moving_im, p_opt, True,
                       mem_limit_mb, num_threads)[1:]
    ...
    for step in range(1, max_step_num+1):
        lcc_val, grad, H = \
                similarity.lcc(fixed_im, moving_im, p, True,
                               mem_limit_mb, num_threads)[1:]
        logging.debug('step: {:d} lcc: {:f}, lambda: {:e}'.format(
            step, lcc_val, l))
        #
        ...
        #
        # parameter update
        delta_v = _update_p(moving_im, R_c_w,
                            p_opt, grad_opt, H_opt, l, p)
        #
        ...
        if step > 1:
            ...
            if nr_of_upscale_steps >= max_nr_of_upscale_steps:
                logging.debug('LM termination: {:d} upscale steps'.format(
                    nr_of_upscale_steps))
                is_finished = True
        if is_finished:
            break
