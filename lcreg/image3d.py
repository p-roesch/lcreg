...
class Image3D(object):
    ...
    """
    def __init__(self, in_data, offset=None, spacing=None,
        ...
        self._mhd_param['DimSize'] = \
            array_to_string(np.array(in_data.shape[::-1],
                                     dtype=np.int32))
    ...
    def _update_geometry(self):
        ...
        self._mhd_param['TransformMatrix'] = \
            array_to_string(self._transform_matrix.T)
        ...
        try:
            ...
            self._voxel_to_world_matrix[:3, :3] = \
                np.dot(self._transform_matrix, np.diag(self._spacing))
            self._world_to_voxel_matrix =\
                np.linalg.inv(self._voxel_to_world_matrix)
    ...
    def shape_array_xyz(self):
        ...
        """
        return np.array(self._data.shape[::-1], dtype=np.uint32)
...
def _raw_to_bcolz(raw_file_name, file_byteorder,
                  dim_size, dtype, root_dir_name, n_bits,
                  grey_range, is_mask, header_size):
    """
    ...
    with open(raw_file_name, 'r') as bin_file:
        ...
        out_slice = np.zeros(shape=(1, dim_size[1], dim_size[0]),
                             dtype=BCOLZ_DTYPE)
        for _ in range(dim_size[2]):
            ...
            rescale_gvals(im_slice, out_slice, g_min, g_max,
                          out_min, out_max, epsilon, is_mask)
            if bcolz_array is None:
                ...
                if (2*expected_len)//2**20 <= BCOLZ_COMPRESSION_LIMIT_MB:
                    c_params = bcolz.cparams(clevel=1,
                                             shuffle=bcolz.NOSHUFFLE)
                bcolz_array = bcolz.carray(out_slice,
...
def import_image(in_file_name, root_dir_name,
                 n_bits=None, grey_range=None, is_mask=False):
    """
    ...
    if param['ElementDataFile'] == 'LIST' or \
            len(param['ElementDataFile'].split()) > 1:
    ...
    if grey_range is not None:
        ...
        if param['BinaryDataByteOrderMSB'] == 'True':
            ...
            im_dtype = '<' + im_dtype
        im_data = np.memmap(raw_file_name, mode='r',
    ...
    logging.debug('{} gVal range: {} - {}, offset: {}'.format(
    ...
    bcolz_array = _raw_to_bcolz(raw_file_name, data_byteorder,
                                dim_size, dtype, root_dir_name,
                                data_bits, grey_range, is_mask,
                                header_size)
    ...
    bcolz_array.attrs['log'] = [
        time.asctime() + ': ' + 'imported from file "' +
        in_file_name + '"',
    ]
    if not is_mask:
...
def export_image(im, file_name):
    ...
    data_file_name = os.path.join(os.path.dirname(file_name),
                                  param['ElementDataFile'])
...
def export_bcolz_image(root_dir_name, out_file_name, take_abs=False):
    ...
    data_file_name = os.path.join(os.path.dirname(out_file_name),
                                  param['ElementDataFile'])
    # store image data slice by slice into raw file
    with open(data_file_name, 'wb') as out_file:
        out_slice = np.zeros((bcolz_array.shape[1], bcolz_array.shape[2]),
                             dtype=np.int16)
...
def create_image(root_dir_name, dim_size, offset=None, spacing=None,
                 transform_matrix=None, mode='w'):
    """
    ...
    expected_len = int(dim_size[0]) * int(dim_size[1]) * int(dim_size[2])
    # create emtpy slice
    im_slice = np.zeros(shape=(1, dim_size[1], dim_size[2]),
                        dtype=BCOLZ_DTYPE)
    ...
    data = bcolz.carray(im_slice, rootdir=root_dir_name,
                        cparams=c_params,
                        expectedlen=expected_len, mode=mode)
    ...
    data.attrs['log'] = [
        time.asctime() + ': ' + 'image created with root_dir_name "' +
        root_dir_name + '", dim_size=' +
...
def empty_image_like(template, root_dir_name):
    ...
    """
    return create_image(root_dir_name, template.data.shape,
                        template.offset, template.spacing,
...
def image_part(source_im, offset_voxel, size_voxel):
    ...
    new_data = source_im.data[offset_voxel[2]:new_max_vox[2],
                              offset_voxel[1]:new_max_vox[1],
                              offset_voxel[0]:new_max_vox[0]]
    # create new image3d with adapted data and geometry
    return Image3D(new_data, new_world_offset, source_im.spacing,
...
def reduce_mhd(in_im_name, out_im_name, skip_3d):
    ...
    in_shape = string_to_array(in_mhd['DimSize'], np.uint32)[::-1]
    in_data_file_name = os.path.join(os.path.dirname(in_im_name),
                                     in_mhd['ElementDataFile'])
    in_data = np.memmap(in_data_file_name, dtype=in_dtype,
                        mode='r', shape=tuple(in_shape))
    out_shape = in_data[::skip_3d[2], ::skip_3d[1], ::skip_3d[0]].shape
    out_spacing = \
        string_to_array(in_mhd['ElementSpacing']) * skip_3d
    out_mhd = in_mhd.copy()
    ...
    out_data = np.memmap(out_raw_name, dtype=out_dtype,
...
def resample_mhd(fixed_mhd_file_name, moving_mhd_file_name,
                 out_mhd_file_name, matrix, offset):
    """
    ...
    out_shape = string_to_array(out_mhd['DimSize'], np.uint32)[::-1]
    out_dtype = _MHD_TYPES[out_mhd['ElementType']][1]
    if out_mhd['BinaryDataByteOrderMSB'] == 'True':
        out_dtype = '>' + out_dtype
    else:
        out_dtype = '<' + out_dtype
    moving_shape = string_to_array(moving_mhd['DimSize'], np.uint32)[::-1]
...
def abs_difference_image_mhd(im1_mhd_file_name, im2_mhd_file_name,
    ...
    if out_mhd['BinaryDataByteOrderMSB'] == 'True':
        ...
        out_dtype = '<' + out_dtype
    im1_shape = string_to_array(im1_mhd['DimSize'], np.uint32)[::-1]
    im1_dtype = _MHD_TYPES[im1_mhd['ElementType']][1]
    if im1_mhd['BinaryDataByteOrderMSB'] == 'True':
        im1_dtype = '>' + im1_dtype
    else:
        im1_dtype = '<' + im1_dtype
    im2_shape = string_to_array(im2_mhd['DimSize'], np.uint32)[::-1]
